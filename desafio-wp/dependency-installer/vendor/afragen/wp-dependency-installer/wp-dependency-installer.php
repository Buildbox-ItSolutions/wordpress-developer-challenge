	...
	class WP_Dependency_Installer {
		...
		public function install( $slug ) {
			...
			if ( $this->is_required( $slug ) ) {
				...
				return [
					...
					'message' => sprintf( esc_html__( '%s foi instalado e ativado.' ), $this->config[ $slug ]['name'] ),
			...
			return [
				'status'  => 'updated',
				/* translators: %s: Plugin name */
				'message' => sprintf( esc_html__( '%s foi instalado.' ), $this->config[ $slug ]['name'] ),
				'source'  => $this->config[ $slug ]['source'],
			];
		}
		...
		public function activate( $slug ) {
			...
			return [
				'status'  => 'updated',
				/* translators: %s: Plugin name */
				'message' => sprintf( esc_html__( '%s foi ativado.' ), $this->config[ $slug ]['name'] ),
				'source'  => $this->config[ $slug ]['source'],
			];
		}
